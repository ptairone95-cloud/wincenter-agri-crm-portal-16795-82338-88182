-- Corrigir função create_commission_for_sale e recriar trigger

-- 1. Corrigir a função create_commission_for_sale
...
BEGIN
  ...
  ELSIF v_rule.base IN ('maintenance', 'revision', 'spraying') THEN
    ...
    v_base_value := COALESCE(v_service_total_value, 0);
  ELSE
    v_base_value := v_sale.gross_value;
  END IF;
...
-- 3. Criar trigger na tabela sales
DROP TRIGGER IF EXISTS trg_after_sale_upsert ON public.sales;
CREATE TRIGGER trg_after_sale_upsert
  AFTER INSERT OR UPDATE ON public.sales
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_sale_create_commission();

-- 4. Processar todas as vendas fechadas existentes que não têm comissão
DO $$
DECLARE
  v_sale_id uuid;
BEGIN
